...
window.addEventListener('DOMContentLoaded', () => {
  ...
  imageEl.addEventListener('touchstart', (event) => {
    imagesTouches = event.touches;
  })

  imageEl.addEventListener('touchmove', (event) => {
    if (imagesTouches.length === 0) return

    //pinch zoom
    if (event.touches.length === 2 && imagesTouches.length === 2) {
      const prevDistance = Math.hypot(
        imagesTouches[0].pageX - imagesTouches[1].pageX,
        imagesTouches[0].pageY - imagesTouches[1].pageY
      )
      ...
      if (newDistance - prevDistance > 5) {
        zoomIn()
        imagesTouches = event.touches
      } else if (prevDistance - newDistance > 5) {
        zoomOut()
        imagesTouches = event.touches
      }
    }
